...
export class AuthController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Retorna access_token (15 min) y refresh_token (7 días)',
  })
  @ApiResponse({
    status: 401,
  ...
  @ApiResponse({
    status: 204,
    description: 'Sesión cerrada — refresh token invalidado',
  })
  @ApiResponse({ status: 401, description: 'No autenticado' })
  logout(@CurrentUser() user: { id: number }, @Ip() ip: string) {
  ...
  @ApiResponse({
    status: 200,
    description: 'Respuesta genérica (no confirma si el email existe)',
  })
