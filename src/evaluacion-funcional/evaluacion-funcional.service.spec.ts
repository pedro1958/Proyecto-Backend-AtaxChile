...
import { EvaluacionFuncional, NivelMovilidad } from './entities/evaluacion-funcional.entity'
...
describe('EvaluacionFuncionalService', () => {
  ...
  beforeEach(async () => {
    ...
    service = module.get<EvaluacionFuncionalService>(EvaluacionFuncionalService)
  ...
  describe('create', () => {
    ...
    it('debe lanzar NotFoundException si el miembro no existe', async () => {
      ...
      await expect(service.create(99, createDto, 10)).rejects.toThrow(NotFoundException)
      await expect(service.create(99, createDto, 10)).rejects.toThrow('Miembro no encontrado')
  ...
  describe('findAllByMiembro', () => {
    it('debe retornar historial ordenado por fecha desc', async () => {
      const evaluacionAnterior = { ...mockEvaluacion, id: 2, fecha: '2024-09-01' }
      evaluacionRepo.find.mockResolvedValue([mockEvaluacion, evaluacionAnterior])
  ...
  describe('findUltimaByMiembro', () => {
    ...
    it('debe lanzar NotFoundException si no hay evaluaciones para el miembro', async () => {
      ...
      await expect(service.findUltimaByMiembro(99)).rejects.toThrow(NotFoundException)
