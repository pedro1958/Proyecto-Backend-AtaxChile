...
import { DiagnosticoClinico, ConfirmacionDiagnostico } from './entities/diagnostico-clinico.entity'
...
describe('DiagnosticoClinicoService', () => {
  ...
  describe('create', () => {
    ...
    it('debe lanzar NotFoundException si el miembro no existe', async () => {
      ...
      await expect(service.create(99, createDto)).rejects.toThrow(NotFoundException)
      await expect(service.create(99, createDto)).rejects.toThrow('Miembro no encontrado')
  ...
  describe('findByMiembro', () => {
    ...
    it('debe lanzar NotFoundException si no existe diagnóstico para el miembro', async () => {
      ...
      await expect(service.findByMiembro(99)).rejects.toThrow(NotFoundException)
  ...
  describe('update', () => {
    it('debe actualizar el diagnóstico existente', async () => {
      diagnosticoRepo.findOneBy.mockResolvedValue({ ...mockDiagnostico })
      diagnosticoRepo.save.mockResolvedValue({ ...mockDiagnostico, subtipo: 'SCA3' })
    ...
    it('debe lanzar NotFoundException si no existe diagnóstico para el miembro', async () => {
      ...
      await expect(service.update(99, { subtipo: 'SCA3' })).rejects.toThrow(NotFoundException)
    })
  })
})
