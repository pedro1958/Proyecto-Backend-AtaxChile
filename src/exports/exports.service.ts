...
export class ExportsService {
  ...
  async generarFichaIndividual(
    ...
    if (user.rol === Rol.TESORERO) {
      throw new ForbiddenException(
        'No tiene permisos para exportar ficha individual',
      );
    }
  ...
  private async obtenerDatos(
    ...
    if (query.tipoAtaxiaId) {
      qb.andWhere('m.tipoAtaxiaId = :tipoAtaxiaId', {
        tipoAtaxiaId: query.tipoAtaxiaId,
      });
    }

    if (query.fechaDesde) {
      qb.andWhere('m.fechaInscripcion >= :fechaDesde', {
        fechaDesde: query.fechaDesde,
      });
    }

    if (query.fechaHasta) {
      qb.andWhere('m.fechaInscripcion <= :fechaHasta', {
    ...
    const rows: MiembroExportRow[] = miembros.map((m) => {
      ...
      if (!esTesorero) {
        ...
        base.ultimaEvaluacion =
          null as unknown as MiembroExportRow['ultimaEvaluacion'];
      }
